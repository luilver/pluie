      ...
      module FormatToDeliveryReport
        def to_csv(options={},reports)
          CSV.generate(options) do |csv|
            ...
            reports.each do |dr|
              csv << [dr.id, I18n.localize(dr.created_at, :format => :date_time).html_safe,dr.msg_id,dr.gateway,dr.body, ActionSmserUtils.show_dlr_status(dr.status),I18n.localize(dr.status_updated_at, :format => :date_time).html_safe,dr.to,I18n.translate("activerecord.models.#{dr.sms_type.tableize.singularize+'_xls'}"),User.find(dr.user_id.to_i).email]
            end
          end
        end
      end
