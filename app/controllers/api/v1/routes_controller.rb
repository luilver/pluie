module Api
  module V1
    class RoutesController < ApiController
      ...
      def by_route_name
        return (render json: {:message=>"name route blank"},status: 404) if params[:route][:name_route].blank?
        if User.current.admin
            if Route.where(:name=>params[:route][:name_route]).any?
              ...
              render json: {:message=>"Name: #{params[:route][:name_route]} invalid"}, status: 422
            end
        else
          render json: {:message=>"You are not admin"}, status: 401
        end
      ...
      def by_email_user
        return (render json: {:message=>"email blank"},status: 404) if params[:route][:email_user].blank?
        if User.current.admin
          if (not User.find_by_email(params[:route][:email_user]).blank?) && (not User.find_by_email(params[:route][:email_user]).routes.blank?)
              ...
              if User.find_by_email(params[:route][:email_user]).blank?
                ...
                render json: {:message => "User not route"},status: 422
              end
          end
        else
          render json: {:message=>"You are not admin"}, status: 401
      ...
      def create
        return (render json: {:message=>"email blank"},status: 404) if params[:route][:email_user].blank?
        return (render json: {:message=>"gateway blank"},status: 404) if params[:route][:gateway_name].blank?
        return (render json: {:message=>"price blank"},status: 404) if params[:route][:price].blank?
        return (render json: {:message=>"name blank"},status: 404) if params[:route][:name].blank?
        if User.current.admin
            if (not User.find_by_email(params[:route][:email_user]).blank?) && (not Gateway.find_by_name(params[:route][:gateway_name]).blank?)
              ...
              if  Gateway.find_by_name(params[:route][:gateway_name]).blank?
                render json: {:message=> "gateway invalid"}, status: 422
              else
                render json: {:message=> "email invalid"}, status: 422
              end
            end
        ...
        if User.current.admin
            ...
            if @route
              ...
              if  (not Gateway.find_by_name(route_params[:gateway_name]).blank?) && (not User.find_by_email(route_params[:email_user]).blank?)
                ...
                render json: {:message=>"route update succefully"}, status: 200
              else
