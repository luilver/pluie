...
class DeliveryReportsControllerTest < ActionController::TestCase
  ...
  setup do
    DeliveryMethods.delivery_options[:gateway_commit_observers] = []
  end

  test "gateway_commit with existing dr" do
    @msg_id = "102010314204056202"
    @dr = DeliveryReport.create(:msg_id => @msg_id, :status => 'ORIGINAL_STATUS')

    DeliveryMethods.delivery_options[:gateway_commit] = {'test_gateway' => SmsTestSetup}
    DeliveryMethods.gateway_commit_observer_add(SmsTestSetup)

    SmsTestSetup.expects(:after_gateway_commit).once.with(){|var| var.is_a?(Array) && var.first.is_a?(DeliveryReport)}
  ...
  test "gateway_commit with multiple records" do
    ...
    @dr = DeliveryReport.create(:msg_id => @msg_id, :status => 'ORIGINAL_STATUS')
    @dr2 = DeliveryReport.create(:msg_id => @msg_id2, :status => 'ORIGINAL_STATUS')
  ...
  test "index with admin_access lambda with right login info" do
    default = DeliveryMethods.delivery_options[:admin_access]
    DeliveryMethods.delivery_options[:admin_access] = SmsTestSetup
    2.times do
      DeliveryReport.create(:msg_id => "idtest_#{rand(10)}")
    end
    ...
    DeliveryMethods.delivery_options[:admin_access] = default
  ...
  test "index with always enabled admin" do
    ...
    DeliveryReportsController.any_instance.stubs(:admin_access_only).returns(true)
    get 'index', :use_route => :action_smser
    assert_response :success
    assert_template :index
  end
